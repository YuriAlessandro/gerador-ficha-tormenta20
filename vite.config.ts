      ...
      output: {
        manualChunks(id) {
          if (id.includes('node_modules')) {
            ...
            if (id.includes('pdf-lib') || id.includes('pdfform')) {
              return 'vendor-pdf';
            }
            if (
            ...
            if (
              id.match(
                /[/\\]node_modules[/\\](react|react-dom|react-router|react-router-dom|react-redux|redux|@reduxjs|redux-persist|scheduler|use-sync-external-store)[/\\]/
              )
            ) {
              return 'vendor-react';
            }
            ...
            if (id.includes('t20-sheet-builder')) {
              return 'vendor-t20-deprecated';
            }
            // NÃO bucketize @3d-dice/dice-box — ele tem dynamic imports internos
            // (Dice, world.offscreen, world.onscreen) que devem ficar como chunks
            // lazy separados, carregados só quando o usuário rola dado.
